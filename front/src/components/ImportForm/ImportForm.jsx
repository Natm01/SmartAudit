...
const ImportForm = ({ projects, onSubmit, loading }) => {
  ...
  const handleMultipleFileSelect = (files, type) => {
    ...
    files.forEach(file => {
      ...
      if (!allowedTypes.includes(ext)) {
        ...
        invalidFiles.push(`${file.name} (demasiado grande)`);
      } else {
  ...
  const removeFile = (e, index, type) => {
    ...
    if (type === 'libroDiario') {
      setFormData(prev => ({ 
        ...prev, 
        libroDiarioFiles: prev.libroDiarioFiles.filter((_, i) => i !== index) 
      }));
    } else {
  ...
  const getFileTypeBadge = (filename) => {
    ...
    return (
      <span className={`px-1.5 py-0.5 text-xs font-medium rounded ${colors[extension] || 'bg-gray-100 text-gray-800'}`}>
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.sumasSaldosFile) {
      newErrors.sumasSaldos = 'Debe seleccionar un archivo de Sumas y Saldos';
    }
              ...
              <div className="space-y-2 max-h-[100px] overflow-y-auto">
                {isMultiple ? (
                  files.map((file, index) => (
                    <div 
                      key={index} 
                      className="flex items-center justify-between bg-gray-50 p-2 rounded-lg group hover:bg-gray-100 transition-colors"
                    >
                      ...
                      <button 
                        type="button" 
                        onClick={(e) => removeFile(e, index, type)}
                        className="ml-2 text-gray-400 hover:text-red-600 transition-all flex-shrink-0 p-1 hover:bg-red-50 rounded"
