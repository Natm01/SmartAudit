...
class MapeoService:
    ...
    async def perform_mapeo(self, azure_file_path: str, erp_hint: str, 
                          execution_id: str) -> Dict[str, Any]:
        """Perform automatic mapeo - SINGLE FILE VERSION"""
        try:
            logger.info(f"Starting mapeo for file: {azure_file_path}")
            ...
            with self.temp_manager.get_local_file(azure_file_path) as local_file:
                ...
                if not mapeo_result.get('success'):
                    return mapeo_result
                
                # Analyze completeness
                completeness = self._analyze_mapping_completeness(local_file, mapeo_result)
                
                mapeo_result.update({
                    'manual_mapping_required': completeness['has_unmapped_fields'],
                    'unmapped_fields_count': completeness['unmapped_count']
                })
