...
class AzureStorageService:
    ...
    def _get_blob_name(self, file_path: str, execution_id: Optional[str] = None, 
                    file_type: Optional[str] = None, stage: Optional[str] = None,
                    description: Optional[str] = None, keep_original_name: bool = True) -> str:
        """
        Generate structured blob name following pattern:
        - For uploads: {executionId}_{originalFilename}_{filetype}.{extension}
        - For mapeo: {executionId}_{description}_{filetype}.{extension}
        - For others: {executionId}_{stage}_{description}_{filetype}.{extension}
        
        Args:
            file_path: Original file path/name
            execution_id: Execution identifier
            file_type: "Je" for Journal Entries, "Sys" for Sumas y Saldos
            stage: Process stage (prediction, processed, result)
            description: Specific description (model, structured, final, header, detail, report)
            keep_original_name: Whether to keep the original filename in the blob name
        ...
        if execution_id:
            ...
            if stage == "upload":
                if keep_original_name:
                    parts.append(original_name_without_ext)
                else:
                    parts.append("upload")
            # Para mapeo, NO duplicar "mapeo" - solo ID + descripción
            elif stage == "mapeo":
                if description:
                    parts.append(description)  # Solo "header", "detail", o "report"
                else:
                    parts.append("mapeo")
            # Para otros stages, incluir stage + description
            else:
                if stage:
                    parts.append(stage)
                if description:
            ...
            # Añadir file_type al final
            if file_type:
                parts.append(file_type)
            
            blob_name = "_".join(parts) + extension
        else:
            # Fallback without execution_id
            timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
            blob_name = f"{original_name_without_ext}_{timestamp}{extension}"
    ...
    def upload_file_chunked(self, local_path: str, container_type: str = "upload", 
        ...
        """
        Upload file using chunked upload with structured naming
        ...
        try:
            ...
            # Determine stage based on container if not provided
            if not stage and container_type in self.containers:
                stage_mapping = {
                    "upload": "upload",
                ...
                stage = stage_mapping.get(container_type, container_type)
    ...
    def upload_from_memory(self, file_data: bytes, filename: str, 
                        ...
                        stage: Optional[str] = None,
                        description: Optional[str] = None,
                        keep_original_name: bool = False) -> str:
        """
